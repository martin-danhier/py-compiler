...
class NObject:
    ...
    def __init__(self, position=0, parent=None):
        ...
        self.parent = parent
        if parent != None:
            self.position = NObjectPosition(
                self.parent.position.file, position)
        ...
        for attr in attr_dict:
            if attr != 'parent':
                ...
                if isinstance(attr_dict[attr], NObject) or isinstance(attr_dict[attr], Identifier):
                    ...
                    dict[attr] = [elem.convert_to_dict()
                                  for elem in attr_dict[attr]]
                else:
    ...
    def get_stack_trace(self):
        ...
        stack_trace = '\tin File "%s", at %s, in %s\n' % (self.position.file, tools.get_coords_from_position(
            self.position.start, self.position.file), self.__class__.__name__)
        if self.parent != None:
        ...
        if match.group('MODULE_TYPE') == 'module':
            if match.group('RETURN_TYPE_A') != None or match.group('RETURN_TYPE_B') != None:
                if match.group('RETURN_TYPE_A') != None:
                    position = NObjectPosition(
                        self.position.file, parent_scan_position + match.start('RETURN_TYPE_A'))
                elif match.group('RETURN_TYPE_B') != None:
                    position = NObjectPosition(
                        self.position.file, parent_scan_position + match.start('RETURN_TYPE_B'))
            ...
            if match.group('PARAMETERS') != None:
                ...
                error.ThrowError(20, NObjectPosition(
                    self.position.file, parent_scan_position + match.start('PARAMETERS')), self.get_stack_trace())
            ...
            # not implemented -> 14
            error.ThrowError(14, NObjectPosition(
                self.position.file, parent_scan_position + match.start()), self.get_stack_trace())
            # TODO
            pass
        else:
            # not module in module -> syntax_error
            error.ThrowError(position=NObjectPosition(self.position.file, parent_scan_position + match.start()),
                             msg="Expected MODULE, but found %s." % match.lastgroup, type="syntax_error")
    ...
    def scan_fun_body(self, match, parent_scan_position):
        ...
        # SWITCH TODO
            ...
            error.ThrowError(11, NObjectPosition(
                self.position.file, parent_scan_position + match.start()), self.get_stack_trace())
    ...
    def is_match_valid(self, match, parent_scan_position):
        ...
        if match.lastgroup == 'MISMATCH':
            ...
            if len(match.group()) > 0 and match.group()[0] == '\"':
                # unclosed string -> 24
                error.ThrowError(24, NObjectPosition(
                    self.position.file, parent_scan_position + match.start()), self.get_stack_trace())
            error.ThrowError(1, NObjectPosition(
                self.position.file, parent_scan_position + match.start()), self.get_stack_trace())
            return False
    ...
    def scan_id(self, source, parent_scan_position):
        for match in re.finditer(full_regex, source):
            if self.is_match_valid(match, parent_scan_position) and match.lastgroup in ('IDENTIFIER', 'PATH'):
                ...
                # error -> 12
                error.ThrowError(12, NObjectPosition(
                    self.position.file, parent_scan_position + match.start()), self.get_stack_trace())
    ...
    def scan_expression(self, source: str, parent_scan_position):
        ...
        source = temp[0]
        # update scan position in order to keep its accuracy
        parent_scan_position += temp[1]
        del(temp)
        ...
        for match in re.finditer(full_regex, source):
            if self.is_match_valid(match, parent_scan_position):
                if value != None:
                    ...
                    error.ThrowError(16, NObjectPosition(
                        self.position.file, parent_scan_position + match.start()), self.get_stack_trace())
                elif match.lastgroup == 'LOGIC_GATE':
                    value = Comparison(match, 'LOG', self,
                                       parent_scan_position)
                elif match.lastgroup == 'LOGIC_NOT':
                    value = Comparison(match, 'NOT', self,
                                       parent_scan_position)
                elif match.lastgroup == 'COMPARISON':
                    value = Comparison(match, 'COMP', self,
                                       parent_scan_position)
                    ...
                    value = Identifier(
                        match.group(), match.start() + parent_scan_position, self)
                elif match.lastgroup == 'STRING_LITERAL':
                    value = Constant('string', match.group().strip(
                        '"'), parent_scan_position + match.start(), self)
                elif match.lastgroup == 'CHAR_LITERAL':
                    value = Constant('char', match.group().strip(
                        '\''), match.start() + parent_scan_position, self)
                elif match.lastgroup in ('INT_DEC_LITERAL', 'INT_HEX_LITERAL', 'INT_DEC_LITERAL'):
                    value = Constant('int', int(match.group()),
                                     match.start() + parent_scan_position, self)
                    ...
                    if value in ('true', 'yep'):
                        ...
                        value = False
                    value = Constant(
                        'bool', value, match.start() + parent_scan_position, self)
...
class Module(NObject):
    ...
    def get_stack_trace(self):
        ...
        stack_trace = '\tin File "%s", at %s, in %s : "%s"\n' % (self.position.file, tools.get_coords_from_position(
            self.position.start, self.position.file), self.__class__.__name__, self.identifier)
        if self.parent != None:
            stack_trace = self.parent.get_stack_trace() + stack_trace
        return stack_trace
    ...
    def __init__(self, args, parent=None, parent_scan_position=0):
        ...
        if isinstance(args, tuple):
            ...
            if parent == None:  # root object, like a file
                ...
                for match in re.finditer(full_regex, args[0]):
                    if match.lastgroup == 'MISMATCH':
                        ...
                        error.ThrowError(1, NObjectPosition(
                            self.position.file, match.start()), self.get_stack_trace())
                    # skip those
            ...
            if parent == None:
                ...
                NObject.__init__(self, args.start() +
                                 parent_scan_position, parent)
                self.identifier = Identifier(args.group('MODULE_ID'), args.start(
                    'MODULE_ID') + parent_scan_position, self)
                self.children = []
                # Process body
                for match in re.finditer(full_regex, args.group('BODY')):
                    if self.is_match_valid(match, parent_scan_position + args.start('BODY')):
                        if self.__class__.__name__ == 'Module':  # in a module, there can only be other modules
                            self.children.append(self.scan_module(
                                match, parent_scan_position + args.start('BODY')))
                        elif self.__class__.__name__ == 'Fun':  # in a function, there can also be instructions, branches, loops
                            self.children.append(self.scan_fun_body(
                                match, parent_scan_position + args.start('BODY')))
                        elif self.__class__.__name__ == 'Class':  # class TODO
                            # not implemented -> 14
                            # no gonna raise etc
...
class Fun(Module):
    ...
    def __init__(self, match, parent, parent_scan_position):
        ...
        if (return_type_a != None and return_type_b != None):
            ...
            error.ThrowError(7, NObjectPosition(
                self.position.file, parent_scan_position + match.start('RETURN_TYPE_A')), self.get_stack_trace())
        elif (return_type_a != None):
            self.return_type = self.scan_id(
                return_type_a, match.start('RETURN_TYPE_A'))
        ...
        for m in re.finditer(full_regex, temp[0]):
            if self.is_match_valid(m, parent_scan_position + m.start() + temp[1] + match.start('PARAMETERS')) and m.lastgroup != 'PARAM_SEPARATOR':
                if m.lastgroup in ('DECLARATION', 'ASSIGNEMENT'):
                    self.parameters.append(self.scan_fun_body(
                        m, parent_scan_position + temp[1] + m.start() + match.start('PARAMETERS')))
                else:
                    # something else -> 25

                    error.ThrowError(25, NObjectPosition(self.position.file, parent_scan_position +
                                                         temp[1] + m.start() + match.start('PARAMETERS')), self.get_stack_trace())
...
class While(NObject):
    ...
    def __init__(self, match, do_while: bool, parent, parent_scan_position):
        ...
        if do_while:
            ...
            prefix = 'WHILE'
        self.loop_condition = self.scan_expression(match.group(
            prefix + '_CONDITION'), parent_scan_position + match.start(prefix + '_CONDITION'))
        self.children = []
        for m in re.finditer(full_regex, match.group(prefix + '_BODY')):
            if self.is_match_valid(m, parent_scan_position + match.start(prefix + '_BODY')):
                self.children.append(self.scan_fun_body(
                    m, parent_scan_position + match.start(prefix + '_BODY')))
    ...
    def __init__(self, match, parent, parent_scan_position):
        ...
        for m in re.finditer(full_regex, match.group('FOR_BODY')):
            if self.is_match_valid(m, parent_scan_position + match.start('FOR_BODY')):
                self.children.append(self.scan_fun_body(
                    m, parent_scan_position + match.start('FOR_BODY')))
        # process start
        for m in re.finditer(full_regex, match.group('FOR_START')):
            if self.is_match_valid(m, parent_scan_position + match.start('FOR_START')):
                if m.lastgroup == 'ASSIGNEMENT':
                    self.start = Assignement(
                        m, self, parent_scan_position + match.start('FOR_START'))
                else:
                    # not an assignement -> 21
                    error.ThrowError(21, NObjectPosition(
                        self.position.file, parent_scan_position + match.start('FOR_START')), self.get_stack_trace())
        # process end
            ...
            for m in re.finditer(full_regex, '%s<(%s)' % (self.start.id, temp[0])):
                if self.is_match_valid(m, parent_scan_position + temp[1] + match.start('FOR_END')):
                    self.condition = Comparison(
                        m, 'COMP', self, parent_scan_position + temp[1] + match.start('FOR_END'), True)
        else:
            self.condition = self.scan_expression(match.group(
                'FOR_END'), parent_scan_position + match.start('FOR_END'))
        # process step
        step = match.group('FOR_STEP')
        if (step == None):
            self.step = Constant(
                'int', 1, parent_scan_position + match.start(), self)
        else:
            self.step = self.scan_expression(
                step, parent_scan_position + match.start('FOR_STEP'))
...
class Declaration(NObject):
    ...
    def __init__(self, match, parent, parent_scan_position):
        ...
        if (decl_type_a not in ['var', None] and decl_type_b != None):
            ...
            error.ThrowError(9, NObjectPosition(
                self.position.file, parent_scan_position + match.start('DECL_ID')), self.get_stack_trace())

        elif (decl_type_a == "var" and decl_type_b != None):
            self.type = self.scan_id(decl_type_b, match.start(
                'DECL_TYPE_B') + parent_scan_position)
        elif (decl_type_a != "var" and decl_type_b == None):
            self.type = self.scan_id(decl_type_a, match.start(
                'DECL_TYPE_A') + parent_scan_position)
        del(decl_type_a, decl_type_b)

        # scan id, it can only be an identifier (regex)
        self.id = Identifier(match.group('DECL_ID'), match.start(
            'DECL_ID') + parent_scan_position, self)
...
class Assignement(NObject):
    ...
    def __init__(self, match, parent=None, parent_scan_position=0):
        NObject.__init__(self, match.start() + parent_scan_position, parent)
        self.id = self.scan_id(match.group(
            'ASSIGN_VAR_ID'), parent_scan_position + match.start('ASSIGN_VAR_ID'))
...
class Call(NObject):
    ...
    def __init__(self, match, parent=None, parent_scan_position=0):
        NObject.__init__(self, match.start() + parent_scan_position, parent)
        self.id = self.scan_id(match.group('CALL_ID'),
                               parent_scan_position + match.start('CALL_ID'))
        ...
        if arguments != None:
            ...
            for m in re.finditer(full_regex, arguments):
                if self.is_match_valid(m, parent_scan_position + m.start() + match.start('CALL_ARGUMENTS')) and m.lastgroup != 'PARAM_SEPARATOR':
                    self.arguments.append(self.scan_expression(
                        m.group(), parent_scan_position + m.start() + match.start('CALL_ARGUMENTS')))
...
class Return(NObject):
    ...
    def __init__(self, match, parent, parent_scan_position):
        NObject.__init__(self, match.start() + parent_scan_position, parent)
        self.return_value = self.scan_expression(match.group(
            'RETURN_VALUE'), parent_scan_position + match.start('RETURN_VALUE'))
...
class Branch(NObject):
    ...
    def __init__(self, match, parent, parent_scan_position):
        ...
        self.if_condition = self.scan_expression(match.group(
            'IF_CONDITION'), parent_scan_position + match.start('IF_CONDITION'))
        # process if_body
        self.if_body = []
        for m in re.finditer(full_regex, match.group('IF_BODY')):
            if self.is_match_valid(m, parent_scan_position):
                self.if_body.append(self.scan_fun_body(
        ...
        if match.group('ELSE_BODY') != None:
            for m in re.finditer(full_regex, match.group('ELSE_BODY')):
                if self.is_match_valid(m, parent_scan_position + match.start('ELSE_BODY')):
                    self.else_body.append(self.scan_fun_body(
...
class Path(NObject):
    ...
    def __init__(self, match, parent, parent_scan_position):
        NObject.__init__(self, match.start() + parent_scan_position, parent)
        # Process right term
        self.right_term = Identifier(match.group('PATH_RIGHT_ID'), match.start(
            'PATH_RIGHT_ID') + parent_scan_position, self)
        # Process left term
        self.left_term = self.scan_id(match.group('PATH_LEFT_ID'), match.start(
            'PATH_LEFT_ID') + parent_scan_position)
...
class Comparison(NObject):
    ...
    def __init__(self, match, prefix, parent=None, parent_scan_position=0, invisible_left_term=False):
        ...
        if invisible_left_term:
            ...
            # -1 because a one appeared somewhere, maybe because it is 0-based ?
            parent_scan_position -= match.start(prefix + '_RIGHT_TERM') + 1
        temp = self.simplify_term(match.group(prefix + '_LEFT_TERM'))
        self.left_term = self.scan_expression(
            temp[0], parent_scan_position + temp[1] + match.start(prefix + '_LEFT_TERM'))
